...
class Game {
  ...
  moveObjects(delta) {
    ...
    this.listOfBullets.forEach(bullet => {
      bullet.move(delta);
    })
  }
  ...
  start(ctx) {
    ...
    for(let i = 0 ; i < numTowers ; i++) {
        ...
        if(currentSize.x < 500) {
          ...
          // ctx.beginPath();
          // ctx.beginPath();
          // ctx.arc(currentGridArea.x +90, currentGridArea.y+40,10,0,2*Math.PI);
          // ctx.stroke();
          // setInterval( () => {
          //   if(currentTower.shouldAttack(this.listOfMonsters)) {
          //     let attack = {
          //       x: currentGridArea.x + 90,
          //       y: currentGridArea.y ,
